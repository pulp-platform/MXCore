...
module mxcore_hwpe_multi_fifo_scale_buffer #(
  parameter int unsigned InputDataWidth      = 512,
  parameter int unsigned OutputDataWidth     = 256,
  ...
  localparam int unsigned SCALE_COUNT = ScaleFactor;
  ...
  logic [$clog2(SCALE_COUNT+1)-1:0]     scale_reuse_count_d, scale_reuse_count_q;
    ...
    if (InputDataWidth > OutputDataWidth) begin
      ...
      if (data_o.ready && pop_fifo_valid[0]) begin
        if (reuse_count_q == ReuseFactor - 1) begin
          reuse_count_d = '0;
          if (scale_reuse_count_q == SCALE_COUNT - 1) begin
...
endmodule : mxcore_hwpe_multi_fifo_scale_buffer